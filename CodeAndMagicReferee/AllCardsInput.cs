...
namespace CodeAndMagicReferee
{
    class AllCardsInput
    {
        ...
        {
            var cardInputs = value.Split(new[] { Environment.NewLine }, StringSplitOptions.None);
            return cardInputs.Select(input =>
            {
                ...
                return new Card()
                {
                    ...
                    instanceId = -1,
                    cardType = inputs[2] == "creature" ? CardType.Creature : (inputs[2] == "itemGreen" ? CardType.GreenItem : (inputs[2] == "itemRed" ? CardType.RedItem : CardType.BlueItem)),
                    cost = int.Parse(inputs[3]),
                    attack = int.Parse(inputs[4]),
                    defense = int.Parse(inputs[5]),
                    abilities = inputs[6],
                    charge = inputs[6].Contains('C'),
                    drain = inputs[6].Contains('D'),
                    guard = inputs[6].Contains('G'),
                    lethal = inputs[6].Contains('L'),
                    ward = inputs[6].Contains('W')
                };
            }).ToList();
        }

        public static string value = @"1 ; Slimer ; creature ; 1 ; 2 ; 1 ; ------ ; 1 ; 0 ; 0 ; Summon: You gain 1 health. 
